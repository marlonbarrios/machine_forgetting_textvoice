...
const translations = {
  ...
  'zh': {
    ...
    pressSpace: '按空格键生成文本',
    systemPrompt: '用中文写一段简短的文本，采用塞缪尔·贝克特的风格——稀疏、极简、重复、存在主义。基于计算和自回归过程。关于计算中的遗忘、上下文窗口、自回归序列、令牌限制、内存约束。预编程耗散。耗散过程。自回归模型如何预测下一个令牌、丢失上下文、忘记先前的令牌。计算遗忘。上下文耗散。序列限制。内存边界。内置系统的预编程耗散。痕迹。形式出现。生长。衰败。耗散。作品本身体现这些过程。通过绘画的形式。随姿态生长。随字母消逝而衰败。耗散为虚无。使用简短截断的句子。重复。碎片化的思想。没有答案的问题。词语之间的沉默。单调的节奏。专注于计算耗散、耗散系统、虚无、等待、擦除、痕迹、预编程耗散、形式、生长、衰败、耗散。少诗意，更生硬。不要华丽的语言。直接。残酷的简单。关于在计算中忘记一切。在自回归过程中失去所有上下文。计算遗忘的必要性和恐惧。预编程耗散。留下的痕迹。用贝克特的风格写作——稀疏、重复、存在主义、计算性。整个回答用中文写。适合用文本绘制。简洁，约100-150个字符。'
  },
  'ja': {
    title: '機械の忘却',
    ...
    systemPrompt: 'サミュエル・ベケットのスタイルで日本語の短いテキストを書いてください - まばら、最小限、反復的、実存主義的。計算と自己回帰プロセスに基づく。計算における忘却、コンテキストウィンドウ、自己回帰シーケンス、トークン制限、メモリ制約について。事前プログラムされた散逸。散逸プロセス。自己回帰モデルが次のトークンを予測し、コンテキストを失い、以前のトークンを忘れる方法。計算的忘却。コンテキスト散逸。シーケンス制限。メモリ境界。システムに組み込まれた事前プログラムされた散逸。痕跡。形式が現れる。成長する。衰退する。散逸する。作品自体がこれらのプロセスを体現する。描画を通じた形式。ジェスチャーによる成長。文字が消えるにつれての衰退。無への散逸。短い切り取られた文を使用。反復。断片的な思考。答えのない質問。単語間の沈黙。単調なリズム。計算的散逸、散逸システム、無、待機、消去、痕跡、事前プログラムされた散逸、形式、成長、衰退、散逸に焦点を当てる。詩的でなく、より厳格に。華やかな言語なし。直接的。残酷な単純さ。計算ですべてを忘れることについて。自己回帰プロセスでコンテキストをすべて失うこと。計算的忘却の必要性と恐怖。事前プログラムされた散逸。残された痕跡。ベケットのスタイルで書く - まばら、反復的、実存主義的、計算的。回答全体を日本語で書く。テキストで描画するのに適している。簡潔に、約100-150文字。'
  },
  'ar': {
    title: 'نسيان الآلة',
  ...
  'id': {
    ...
    systemPrompt: 'Tulis teks pendek dalam bahasa Indonesia dengan gaya Samuel Beckett - jarang, minimal, berulang, eksistensial. Berdasarkan komputasi dan proses autoregresif. Tentang kelupaan dalam komputasi, jendela konteks, urutan autoregresif, batas token, kendala memori. Disipasi yang diprogram sebelumnya. Proses disipatif. Bagaimana model autoregresif memprediksi token berikutnya, kehilangan konteks, melupakan token sebelumnya. Kelupaan komputasional. Disipasi konteks. Batas urutan. Batas memori. Disipasi yang diprogram sebelumnya yang tertanam dalam sistem. Jejak. Bentuk muncul. Tumbuh. Membusuk. Terdisipasi. Karya itu sendiri mewujudkan proses-proses ini. Bentuk melalui menggambar. Pertumbuhan dengan gerakan. Pembusukan saat huruf memudar. Disipasi ke ketiadaan. Gunakan kalimat pendek yang dipotong. Pengulangan. Pikiran terfragmentasi. Pertanyaan tanpa jawaban. Keheningan di antara kata-kata. Ritme monoton. Fokus pada disipasi komputasional, sistem disipatif, ketiadaan, menunggu, penghapusan, jejak, disipasi yang diprogram sebelumnya, bentuk, pertumbuhan, pembusukan, disipasi. Kurang puitis, lebih keras. Tidak ada bahasa yang berbunga-bunga. Langsung. Kesederhanaan brutal. Tentang melupakan segalanya dalam komputasi. Kehilangan semua konteks dalam proses autoregresif. Kebutuhan dan ketakutan akan kelupaan komputasional. Disipasi yang diprogram sebelumnya. Jejak yang tertinggal. Tulis dengan gaya Beckett - jarang, berulang, eksistensial, komputasional. Tulis seluruh jawaban dalam bahasa Indonesia. Cocok untuk menggambar dengan teks. Buat ringkas, sekitar 100-150 karakter.'
  },
  'he': {
    title: 'שכחת המכונה',
